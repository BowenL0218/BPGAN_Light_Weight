...
output_path = os.path.join("./BP_Results/", opt.name+"_LR")
...
for i, data in enumerate(dataset):
    ...
    with torch.no_grad():
        ...
        U,s,V = torch.svd_lowrank(latent_vector.view(lv_size[1], lv_size[2]*lv_size[3]), q=8)
        latent_vector = torch.mm(torch.mm(U, torch.diag(s)), V.T)
