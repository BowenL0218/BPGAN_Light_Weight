...
class Bpgan_VGGLoss(nn.Module):
    def __init__(self, d=40,sampling_ratio = 16000, n_fft=512, n_mels = 128,path = None, cuda=True):
        super(Bpgan_VGGLoss, self).__init__()
        if cuda:
            self.vgg = Bpgan_VGGExtractor(d=d).cuda()
        else:
            self.vgg = Bpgan_VGGExtractor(d=d)
        ...
        if cuda:
            ...
            self.Transform_tensor = torch.Tensor(C)
        if path != None:
