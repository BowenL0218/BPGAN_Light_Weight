...
num_weight_centers_encoder = 64
#num_weight_centers_decoder = 8
encoder_sparsity = 0.7
decoder_sparsity = 1.0
...
total_params = list(model.netE.parameters()) 
...
full_modules = []
module_idx = 0
for name, module in model.netE.named_modules():
...
for e in range(1):
    for i, weight in reversed(list(enumerate(total_params))):
        ...
        if not is_bias and len(weight.view(-1)) > num_weight_centers:
            with torch.no_grad():
                """
                Find centroids
                """
            ...
            for k, data in enumerate(dataset):
                ...
                for j in range(ADMM_iter_Q):
                    ...
                    ssim_loss = 1 - ms_ssim((generated_img+1)/2, (input_label+1)/2, data_range=1)
                    Com_loss =  alpha * (mse_loss + 0.1*feat_loss) +  mu / 2.0 * torch.norm(weight - Z + eta, 2) ** 2 
                    if k%50==0 and j % 10 == 0:
                        print('{:3d} {:3d} MSE: {:0.4f} MS-SSIM Loss: {:0.4f} Feature Loss: {:0.4f} ADMM Loss: {:0.4f}'.format(k+1, j+1, mse_loss.item(), ssim_loss.item(), feat_loss.item(), Com_loss.item())) 
